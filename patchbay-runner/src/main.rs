...
//!
//! This binary requires Linux and will not compile on other platforms.

// On non-Linux platforms, provide an empty main that prints an error.
#[cfg(not(target_os = "linux"))]
fn main() {
    eprintln!("patchbay-runner requires Linux (network namespaces).");
    eprintln!("Use patchbay-vm on macOS to run simulations in a VM.");
    std::process::exit(1);
}

// All the actual implementation is Linux-only.
#[cfg(target_os = "linux")]
mod fmt_log;
#[cfg(target_os = "linux")]
mod sim;
...
mod implementation {
    ...
    pub async fn tokio_main() -> Result<()> {
        ...
        match cli.command {
            ...
            Command::FmtLog { file, follow } => super::fmt_log::run(file.as_deref(), follow),
    ...
    pub fn expand_sim_glob(root: &std::path::Path, pattern: &str) -> Result<Vec<PathBuf>> {
    ...
    pub fn find_patchbay_toml(start: &std::path::Path) -> Option<PathBuf> {
    ...
    pub fn inspect_dir(work_dir: &std::path::Path) -> PathBuf {
        work_dir.join("inspect")
    }

    pub fn inspect_session_path(work_dir: &std::path::Path, prefix: &str) -> PathBuf {
