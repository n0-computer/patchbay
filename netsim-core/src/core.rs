...
pub(crate) struct DnsEntries {
    ...
    /// Directory for generated hosts files.
...
impl DnsEntries {
    ...
    /// Returns the path to the generated hosts file for a device namespace,
    /// or `None` if no entries exist for it.
    pub fn hosts_path_for(&self, device_id: NodeId) -> Option<PathBuf> {
        let has_global = !self.global.is_empty();
        let has_device = self
            .per_device
            .get(&device_id)
            .map_or(false, |v| !v.is_empty());
        if has_global || has_device {
            Some(self.hosts_dir.join(format!("{}.hosts", device_id.0)))
        } else {
            None
        }
    }
    ...
    pub fn write_hosts_file(&self, device_id: NodeId) -> Result<()> {
        let path = match self.hosts_path_for(device_id) {
            Some(p) => p,
            None => return Ok(()),
        };
        std::fs::create_dir_all(&self.hosts_dir)
            .context("create hosts dir")?;
        ...
        std::fs::write(&path, buf.as_bytes()).with_context(|| format!("write {}", path.display()))?;
        Ok(())
    }

    /// Rewrites hosts files for all devices that have entries.
    ...
    pub fn resolve(&self, device_id: Option<NodeId>, name: &str) -> Option<IpAddr> {
        // Check device-specific entries first.
...
impl NetworkCore {
    ...
    pub(crate) fn new(cfg: CoreConfig) -> Self {
        let dns = DnsEntries::new(&cfg.prefix);
        let mut core = Self {
